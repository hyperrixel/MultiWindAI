...
def make_records(datapoints, for_train=True):
    ...
    for datapoint in datapoints.values():
        ...
        records.append(DataRow(x_core, y=None))
...
forecasts_train, measurements_train = get_raw_data(TRAIN_ID)
...
for epoch in range(1, NUM_EPOCHS + 1):
    print('\r{}/{} epoch: shuffling data'.format(epoch, NUM_EPOCHS), end='')
    shuffle(data_train)
    epoch_loss = 0.0
    batch_count = 0
    model.train()
    ...
    epoch_loss /= len_train_data
    print('\r{}/{} epoch finished with total average loss {:0.6f}'
          .format(epoch, NUM_EPOCHS, epoch_loss))
    if epoch_loss < best_loss:
        best_loss = epoch_loss
        if epoch_loss < 0.075:
            filename = 'model_1_full_{:03d}e_{:0.4f}l.pth'.format(epoch, epoch_loss)
            torch.save(model.state_dict(), filename)
            print('[.] Model saved to file: {}'.format(filename))
